...
class Discretizer:
    def __init__(self, timestep=0.8, store_masks=True, impute_strategy='zero', start_time='zero',
        ...
        print("os.path.dirname(__file__)", os.path.dirname(__file__)) # get current path
        ...
        with open(config_path) as f:
            ...
            self._is_categorical_channel = config['is_categorical_channel'] # 查抄分类variables
        ...
        print("self._timestep", self._timestep)
        print("self._store_masks", self._store_masks)
        print("self._id_to_channel", self._id_to_channel)
    ...
    def transform(self, X, header=None, end=None):
        ...
        ts = [float(row[0]) for row in X] # get timesseries
        # print("ts", ts)
        for i in range(len(ts) - 1):
            assert ts[i] < ts[i+1] + eps # make sure that no two ts are same
        ...
        for row in X:
            ...
            for j in range(1, len(row)):
                ...
                channel_id = self._channel_to_id[channel]
                # print("channel: ", channel, "channel_id: ", channel_id)
